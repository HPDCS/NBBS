...
ALLOCATORS = $(filter-out Makefile nballoc.mk hoard 1lvl-ll, $(subst $(BASE_ALLOCATORS)/,  ,  $(PATH_ALLOCATORS))) 
...
$(TARGET)-%-nb: main.c #$(BASE_ALLOCATORS)/$(TARGET)-%-nb/nballoc.o
	@echo compiling for $@
	$(CC) $(FLAGS) main.c  -I../../utils  -I$(abspath ../../allocators/$*-nb) -L$(abspath ../../allocators/$*-nb) -l$*-nb  -o $(TARGET)-$*-nb -DALLOCATOR=$*-nb -D'TO_BE_REPLACED_MALLOC(x)=allocate(x)' -D'TO_BE_REPLACED_FREE(x)=deallocate(x)' -lpthread -D'ALLOCATOR_NAME="$*-nb"'

$(TARGET)-%-sl: main.c #$(BASE_ALLOCATORS)/$(TARGET)-%-sl/nballoc.o
	@echo compiling for $@
	$(CC) $(FLAGS) main.c  -I../../utils  -I$(abspath ../../allocators/$*-sl) -L$(abspath ../../allocators/$*-sl) -l$*-sl -o $(TARGET)-$*-sl -DALLOCATOR=$*-sl -D'TO_BE_REPLACED_MALLOC(y)=allocate(y)' -D'TO_BE_REPLACED_FREE(x,y)=deallocate(x,y)' -lpthread -lnuma -D'ALLOCATOR_NAME="$*-sl"'
