...
void parallel_try(){
	unsigned int i, j, k, tentativi, req_blocks, cur_block_size, offset;
	size_t scelta;
	unsigned int tmp = 0;
	...
	i = j = req_blocks = cur_block_size = offset = 0;
...
__attribute__((constructor(400))) void pre_main2(int argc, char**argv){
	...
	if(argc!=4){
		printf("usage: ./a.out <number of threads> <min size> <max size>\n");
		exit(0);
	}
