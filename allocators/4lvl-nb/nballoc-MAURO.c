...
static node* free_tree; //array che rappresenta l'albero, tree[0] è dummy! l'albero inizia a tree[1].
static node_container* containers; //array di container In Numa questo sarà uno specifico tree (di base tree[getMyNUMANode])
static unsigned long long overall_memory_size;
static unsigned long long overall_memory_pages;
...
static unsigned int number_of_container;
...
void init_BANANA(unsigned long long number_of_nodes);
...
__attribute__((constructor(1))) void pre_init(int argc, char**argv) {
	printf("Sono nella funzione di pre-init\n");
	if(argc!=3){
...
void free_node(void* n){
	...
	free_node_(&tree[free_tree[pos].pos],&ROOT);
#ifdef DEBUG
...
void init_BANANA(unsigned long long levels){
	number_of_nodes = (1<<levels) - 1;
	printf("NON NUMA: %d\n", number_of_nodes);
	printf("NON NUMA:\n");
...
void init(unsigned long long levels){
	...
	tree 			= mmap(NULL,(1+number_of_nodes)*sizeof(node), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	containers 		= mmap(NULL,(number_of_nodes-1)*sizeof(node_container), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	free_tree  		= mmap(NULL,(number_of_leaves)*sizeof(node), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
     
	int i=0;
     for(i;i < number_of_leaves; i++ )
     	free_tree[i].pos = 0;
	
	if(overall_memory==MAP_FAILED || tree==MAP_FAILED || containers==MAP_FAILED || containers==MAP_FAILED){
		puts("Failing allocating structures\n");
		abort();
	}
