...
#define LEAF_FULL_MASK (unsigned long long) (0x1F)
#define LEFT  ((unsigned long long)(0x2))
#define RIGHT  ((unsigned long long)(0x1))
#define COAL_LEFT  ((unsigned long long)(0x8))
#define COAL_RIGHT  ((unsigned long long)(0x4))
#define LOCK_LEAF ((unsigned long long)(0x13))
#define TOTAL ((unsigned long long)(0xffffffffffffffff))
#define ABORT TOTAL
#define LOCK_LEAF_MASK (unsigned long long) (0x13)
#define LOCK_NOT_LEAF_MASK (unsigned long long) (0x1)

#define MASK_NODES_OCCUPIED (((16LLU) | (16LLU)<<5 | (16LLU)<<10 | (16LLU)<<15 | (16LLU)<<20 | (16LLU)<<25 | (16LLU)<<30 | (16LLU)<<35) << 7)	
...
#define BITMASK_LEAF(val, pos)				((0x1f)  & ( ((val) >> ((LEAF_START_POSITION-1) + (5 * ((pos) - (LEAF_START_POSITION))))) ))	
...
#define VAL_OF_NODE(n) ((unsigned long long) (n->container_pos<LEAF_START_POSITION ) ? ((n->container->nodes & (0x1 << (n->container_pos-1))) >> (n->container_pos-1)) : ((n->container->nodes & (LEAF_FULL_MASK << ((LEAF_START_POSITION-1) + (5 * ((n->container_pos-1) - (LEAF_START_POSITION-1))))))) >> ((LEAF_START_POSITION-1) + (5 * ((n->container_pos-1) - (LEAF_START_POSITION-1)))))
...
#define parent(n) 	(tree[(unsigned int)(((n)->pos)/(2))])
#define parent_index(n)  ((unsigned int)(((n)->pos)/(2)))
...
bool check_parent(node* n){
	...
	do{
		...
		if(BITMASK_LEAF(new_val, parent_fix->container_pos) & 0x10){
