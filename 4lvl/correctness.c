...
int main(int argc, char**argv){
	...
	unsigned long long requested = atol(argv[2]);
	
	init(requested);
	printf("_______\n");
	write_on_a_file_in_ampiezza_start();
	...
	printf("Leaves really allocated: %u\n", count_occupied_leaves());
	
	//if(*node_allocated < number_of_leaves)
		write_on_a_file_in_ampiezza();
#endif
