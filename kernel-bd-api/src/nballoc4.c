...
void* bd_xx_malloc(size_t byte){
    ...
    if(mytid == -1){
		mytid = __sync_fetch_and_add(&partecipants, 1);
    }
	...
	actual = started_at = starting_node + (mytid) * ((last_node - starting_node + 1)/partecipants);
	...
	do{
		if((failed_at = alloc(&tree[actual])) == 0){
#ifdef DEBUG
